 ...
 $listG = array();
    ...
    public function editAction() {
       ...
       /* echo '<pre>';
        ...
        $listeSelect=array();
        if($user->isAdmin==3){
            $listeSelect= json_decode($user->gouvernorat,1);
              $gouvernorat = $this->getGestionUserTable()->getListeListsGouvernorat();
            $listG = array();
            foreach ($gouvernorat as $gouv) {
                $listG[] = array('id'=>$gouv['id'],'nam'=>$gouv['nom'] . " / " . $gouv['nomArabe']);
        ...
        if($user->isAdmin==4){
            $listeSelect= json_decode($user->delegations,1);
             $listedelegation = $this->getGestionUserTable()->getListeDelegation();
            $listefinalDelegation = array();
            foreach ($listedelegation as $deleg) {
                $listefinalDelegation[$deleg['idGouvernorat']][] = $deleg;
        ...
        if ($request->isPost()) {
            ...
            if (0) {
                ...
                if (1) {
                    ...
                    $usermodif->pwd = $user->pwdc;
                    $usermodif->pwdc = $user->pwdc;
                     $usermodif->etat = $user->etat;
                     
                      $usermodif->adresseUser = $user->adresseUser;
                       $usermodif->dateNaisUser = $user->dateNaisUser;
                        $usermodif->lieuNaisUser = $user->lieuNaisUser;
                         $usermodif->telUser = $user->telUser;
                          $usermodif->image = $user->image;
                     
                     
                    $usermodif->delegations = $user->delegations;
                    $usermodif->gouvernorat =$user->gouvernorat;
                    
                    if ($userPost->isAdmin == 4) {
                        $usermodif->delegations = json_encode($listeSelectDelegation);
                    }
                    
                    if ($userPost->isAdmin == 3) {
                        $usermodif->gouvernorat = json_encode($listeSelectGovernora);
                    }
        ...
        $view = new ViewModel(array('idUser' => $id, 'form' => $form,'isAdmin'=>$user->isAdmin,"listeSelect"=>$listeSelect,
            'listedelegation' => $listefinalDelegation,'listG'=>$listG));
        return $view;
    }
