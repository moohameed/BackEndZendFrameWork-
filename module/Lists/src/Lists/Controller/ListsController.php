...
//$this->getListsTable()->getListeLists();
    ...
    public function listswsAction() {
      ...
      public function reslistswsAction() {
        // ini_set('display_errors', '1');
        $id = (int) $this->params('id');
		$type = (int) $this->params('type');
        ...
        if ($id) {
            // echo 'id---->'.$id;
            echo $this->getListsTable()->getResListeListsws($id, $type);
            /* if($type){
              echo '-----type---->'.$type;
              } */
        }
    ...
    public function adminlistswsAction() {
        // ini_set('display_errors', '1');
        $id = (int) $this->params('id');
        ...
        if ($id) {
            // echo 'id---->'.$id;
            echo $this->getListsTable()->getAdminListeListsws($id, $type);
            /* if($type){
              echo '-----type---->'.$type;
              } */
        }
    ...
    public function editAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                $dataPoste = $request->getPost();
                $nbrUsers = sizeof($dataPoste->usersNon);
               
               

                if ($nbrUsers > 0) {
                    $listeUsersPost=array();
                    for ($i = 0; $i <$nbrUsers; $i++) {
                        $usersp['nom']=$dataPoste->usersNon[$i];
                        $usersp['post']=$dataPoste->usersPost[$i];
                        $listeUsersPost[]=$usersp;
                    }
                    
                     $lists->list_users = json_encode($listeUsersPost);
                }
        ...
        return array(
            'id' => $id,
            'form' => $form,
            'listeusers'=> json_decode($lists->list_users,1),
        );
    }
